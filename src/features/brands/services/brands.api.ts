...
export async function fetchBrands(
    ...
    try {
        const { data } = await catalogClient.get<BrandListSuccessResponse>(API_ROUTES.BRANDS.LIST, {
            params,
            signal,
            timeout: 10000,
        })
...
export async function fetchBrand(id: string, signal?: AbortSignal): Promise<Brand> {
    ...
    try {
        const { data } = await catalogClient.get<BrandSuccessResponse>(
            API_ROUTES.BRANDS.DETAILS(id),
            { signal },
        )
        return data.data
    } catch (err) {
...
export async function createBrand(
    ...
    try {
        const { data } = await catalogClient.post<BrandSuccessResponse>(
            API_ROUTES.BRANDS.CREATE,
            payload,
            { signal },
        )
        return data.data
...
export async function updateBrand(
    ...
    try {
        const { data } = await catalogClient.put<Brand>(API_ROUTES.BRANDS.UPDATE(id), payload, {
            signal,
        })
        return data
    } catch (err) {
